...
},{"../.option.spectrum.json":17}],37:[function(require,module,exports){
(function (Buffer){
...
},{"../.option.spectrum.json":17,"buffer":2}],38:[function(require,module,exports){
function chit_setTag(t){$("#chit_imagearea").empty();let i=$("#chit_password").val();for(item of ddf.images.imageList)ddf.images.tagInfos[item]?"（全て）"!=t&&!ddf.images.tagInfos[item].tags.includes(t)||""!=ddf.images.tagInfos[item].password&&ddf.images.tagInfos[item].password!=i||$("#chit_imagearea").append($(`<div><img src="${ddf.base_url+item}" /></div>`)):"（全て）"==t&&$("#chit_imagearea").append($(`<div><img src="${ddf.base_url+item}" /></div>`))}function chit_show(t){$("#window_chit").show().css("zIndex",151),$(".draggable:not(#window_chit)").css("zIndex",150),$("#chit_image").show(),$("#chit_imageSelect").hide();var i;index=0,reg=/^(\d+)$/;for(item in ddf.characters)(v=reg.exec(ddf.characters[item].data.name))&&(index=Math.max(index,parseInt(v[1])));i={type:"chit",width:1,height:1,imgId:"",draggable:!0,imageUrl:"./image/defaultImageSet/pawn/pawnBlack.png",rotation:0,info:"",x:0,y:0},$("#chit_imgId").val(i.imgId),$("#chit_width").val(i.width),$("#chit_height").val(i.height),$("#chit_info").val(i.info),$("#chit_imageUrl").val(i.imageUrl),chit_previewUpdate()}function chit_previewUpdate(){zoom=Math.min(1,1/$("#chit_width").val(),1/$("#chit_height").val()),$("#chit_preview").css({width:50*$("#chit_width").val(),height:50*$("#chit_height").val(),transform:`scale(${zoom})`}),$("#chit_preview .inner").css({backgroundImage:"url("+ddf.base_url+$("#chit_imageUrl").val()+")"})}$("#btn_createchit").on("click",t=>{chit_show("")}),$("#window_chit input").on("change",chit_previewUpdate),$(document).on("click","#chit_imagearea div img",t=>{let i=$(t.currentTarget).attr("src");$("#chit_imageUrl").val(i.replace(ddf.base_url,"")),$("#chit_preview .inner").css("backgroundImage",`url(${i})`)}),$("#chit_imageChange").on("click",t=>{ddf.getImageTagsAndImageList().then(t=>{tagList=["キャラクター画像"],ddf.images=t;for(item of ddf.images.imageList)if(ddf.images.tagInfos[item])for(tag of ddf.images.tagInfos[item].tags)""!=tag&&(tagList.includes(tag)||tagList.push(tag));tagList.push("（全て）"),$("#chit_tagbox").empty();for(item of tagList)$("#chit_tagbox").append($(`<option>${encode(item)}</option>`));$("#chit_tagbox").append($(`<option>${encode(item)}</option>`)),chit_setTag(tagList[0])}),$("#chit_image").hide(),$("#chit_imageSelect").show()}),ddf.cmd.chit_show=chit_show,$("#chit_btnpassword").on("click",t=>{$("#chit_btnpassword").hide(),$("#chit_password").show().focus()}),$("#chit_password").on("focusout",t=>{$("#chit_btnpassword").show(),$("#chit_password").hide(),chit_setTag($("#chit_tagbox").val())}).on("keydown",t=>{13==t.keyCode&&$("#chit_password").blur()}),$("#chit_tagbox").on("change",t=>{chit_setTag($("#chit_tagbox").val())}),$("#chit_close, #chit_close2").on("click",t=>{$("#window_chit").hide()});var click={x:0,y:0};$("#chit_preview").draggable({start:t=>{click.x=t.clientX,click.y=t.clientY},helper:()=>{let t=$("#chit_preview").clone();return t.addClass("chitFrame"),t.css("transform",""),t.appendTo("#mapSurface"),t},drag:(t,i)=>{var a=ddf.roomState.zoom,e=i.originalPosition;i.position={left:(t.clientX-click.x+e.left)/a,top:(t.clientY-click.y+e.top)/a},ddf.roomState.viewStateInfo.isSnapMovablePiece&&(ddf.roomState.mapData.isAlternately&&ddf.roomState.mapData.gridInterval%2==1?1&Math.floor(i.position.top/50/ddf.roomState.mapData.gridInterval)?i.position={left:25*(1^(1|Math.floor(i.position.left/25))),top:50*Math.floor(i.position.top/50)}:i.position={left:25*(1|Math.floor(i.position.left/25)),top:50*Math.floor(i.position.top/50)}:i.position={left:50*Math.floor(i.position.left/50),top:50*Math.floor(i.position.top/50)})},stop:(t,i)=>{character={type:"chit",width:$("#chit_width").val(),height:$("#chit_height").val(),info:$("#chit_info").val(),imageUrl:$("#chit_imageUrl").val(),imgId:"",rotation:0,info:$("#chit_info").val(),x:i.position.left/50,y:i.position.top/50},ddf.addCharacter(character),$("#window_chit").hide()}});

},{}],39:[function(require,module,exports){
function diceBotTable_show(e,t,d){tableInfo=ddf.tableInfos.find(((e,t)=>d=>d.gameType==e&&d.command==t)(e,t)),tableInfo||(tableInfo={command:"",dice:"",fileName:"",gameType:"",table:[],title:""}),$("#diceBotTable_edit_originalGameType").val(tableInfo.gameType),$("#diceBotTable_edit_originalCommand").val(tableInfo.command),$("#diceBotTable_edit_copy").val(d),$("#diceBotTable_edit_command").val(tableInfo.command),$("#diceBotTable_edit_dice").val(tableInfo.dice),$("#diceBotTable_edit_titletext").val(tableInfo.title),$("#diceBotTable_edit_gameType").val(""==tableInfo.gameType?"DiceBot":tableInfo.gameType),$("#diceBotTable_edit_table").val(tableInfo.table.map(e=>e[0]+":"+e[1]).join("\n")),$("#window_diceBotTable_edit").show().css("zIndex",151),$(".draggable:not(#window_diceBotTable_edit)").css("zIndex",150)}$("#btn_diceBotTable").on("click",e=>{ddf.getBotTableInfos().then(e=>{ddf.tableInfos=e.tableInfos,$("#window_diceBotTable table tbody").empty();for(item of e.tableInfos)tr="<tr>",tr+=`<td><button class="edit" command="${item.command}" gameType="${item.gameType}">変更</button></td>`,tr+=`<td><button class="copy" command="${item.command}" gameType="${item.gameType}">コピー</button></td>`,tr+=`<td>${encode(ddf.util.getDiceBotName(""==item.gameType?"DiceBot":item.gameType))}</td>`,tr+=`<td>${encode(item.command)}</td>`,tr+=`<td>${encode(item.title)}</td>`,tr+=`<td><button class="delete"  command="${item.command}" gameType="${item.gameType}">削除</button></td>`,tr+="</tr>",$("#window_diceBotTable table tbody").append($(tr))}),$("#window_diceBotTable").show().css("zIndex",151),$(".draggable:not(#window_diceBotTable)").css("zIndex",150)}),$(document).on("click","#window_diceBotTable button.edit",e=>{diceBotTable_show($(e.target).attr("gameType"),$(e.target).attr("command"),!1)}),$(document).on("click","#window_diceBotTable button.copy",e=>{diceBotTable_show($(e.target).attr("gameType"),$(e.target).attr("command"),!0)}),$(document).on("click","#window_diceBotTable button.delete",e=>{confirm($(e.target).attr("command")+"を削除します、よろしいですか？")&&(ddf.removeBotTable($(e.target).attr("command")),$("#window_diceBotTable table tbody").empty(),ddf.getBotTableInfos().then(e=>{ddf.tableInfos=e.tableInfos,$("#window_diceBotTable table tbody").empty();for(item of e.tableInfos)tr="<tr>",tr+=`<td><button class="edit" command="${item.command}" gameType="${item.gameType}">変更</button></td>`,tr+=`<td><button class="copy" command="${item.command}" gameType="${item.gameType}">コピー</button></td>`,tr+=`<td>${encode(ddf.util.getDiceBotName(""==item.gameType?"DiceBot":item.gameType))}</td>`,tr+=`<td>${encode(item.command)}</td>`,tr+=`<td>${encode(item.title)}</td>`,tr+=`<td><button class="delete"  command="${item.command}" gameType="${item.gameType}">削除</button></td>`,tr+="</tr>",$("#window_diceBotTable table tbody").append($(tr))}))}),$("#diceBotTable_create").on("click",e=>{diceBotTable_show("","",!0)}),$("#diceBotTable_close, #diceBotTable_close2").on("click",e=>{$("#window_diceBotTable").hide()}),$("#diceBotTable_edit_close, #diceBotTable_edit_close2").on("click",e=>{$("#window_diceBotTable_edit").hide()}),$("#diceBotTable_edit_sample").on("click",e=>{$("#diceBotTable_edit_dice").val("2d6"),$("#diceBotTable_edit_titletext").val("表サンプル"),$("#diceBotTable_edit_table").val("2:「コマンド名」をチャットに入力することで、\\n表のロールができるようになります。\n3:この例では「SAMPLE」と入力すれば\\n実行できるようになります。\n4:表のフォーマットは\\nまさにここに書いてある通り、\n5:（数値）:（メッセージ）\n6:になります。\n7:「コマンド」をチャットで発言すると\\n「ダイス」に記載したダイスを元にランダム選択されます。\n8:ダイス目に合致する値が表に無い場合は空文字になります。\n9:悩むより一度追加してみるのが早いでしょう。\n10:他の人も使える便利な表が出来たら\\n皆で共有してあげてくださいね！\n11:そろそろ\\n書く事無くなってきましたね…\n12:以上です。\n")}),$("#diceBotTable_edit_send").on("click",e=>{gameType=$("#diceBotTable_edit_gameType").val(),"DiceBot"==gameType&&(gameType=""),"false"==$("#diceBotTable_edit_copy").val()?ddf.changeBotTable($("#diceBotTable_edit_originalGameType").val(),$("#diceBotTable_edit_originalCommand").val(),gameType,$("#diceBotTable_edit_titletext").val(),$("#diceBotTable_edit_command").val(),$("#diceBotTable_edit_dice").val(),$("#diceBotTable_edit_table").val()).then(e=>{"OK"==e.resultText?(ddf.sendChatMessage(0,"dummy\t","###CutInCommand:getDiceBotInfos###{}","00aa00",!0),$("#window_diceBotTable_edit").hide(),$("#window_diceBotTable table tbody").empty(),ddf.getBotTableInfos().then(e=>{ddf.tableInfos=e.tableInfos,$("#window_diceBotTable table tbody").empty();for(item of e.tableInfos)tr="<tr>",tr+=`<td><button class="edit" command="${item.command}" gameType="${item.gameType}">変更</button></td>`,tr+=`<td><button class="copy" command="${item.command}" gameType="${item.gameType}">コピー</button></td>`,tr+=`<td>${encode(ddf.util.getDiceBotName(""==item.gameType?"DiceBot":item.gameType))}</td>`,tr+=`<td>${encode(item.command)}</td>`,tr+=`<td>${encode(item.title)}</td>`,tr+=`<td><button class="delete"  command="${item.command}" gameType="${item.gameType}">削除</button></td>`,tr+="</tr>",$("#window_diceBotTable table tbody").append($(tr))})):alert(e.resultText)}):ddf.addBotTable(gameType,$("#diceBotTable_edit_titletext").val(),$("#diceBotTable_edit_command").val(),$("#diceBotTable_edit_dice").val(),$("#diceBotTable_edit_table").val()).then(e=>{"OK"==e.resultText?(ddf.sendChatMessage(0,"dummy\t","###CutInCommand:getDiceBotInfos###{}","00aa00",!0),$("#window_diceBotTable_edit").hide(),$("#window_diceBotTable table tbody").empty(),ddf.getBotTableInfos().then(e=>{ddf.tableInfos=e.tableInfos,$("#window_diceBotTable table tbody").empty();for(item of e.tableInfos)tr="<tr>",tr+=`<td><button class="edit" command="${item.command}" gameType="${item.gameType}">変更</button></td>`,tr+=`<td><button class="copy" command="${item.command}" gameType="${item.gameType}">コピー</button></td>`,tr+=`<td>${encode(ddf.util.getDiceBotName(""==item.gameType?"DiceBot":item.gameType))}</td>`,tr+=`<td>${encode(item.command)}</td>`,tr+=`<td>${encode(item.title)}</td>`,tr+=`<td><button class="delete"  command="${item.command}" gameType="${item.gameType}">削除</button></td>`,tr+="</tr>",$("#window_diceBotTable table tbody").append($(tr))})):alert(e.resultText)})});

},{}],40:[function(require,module,exports){
function safeDragDestroy(){try{$(".floorTileFrame.draggableObj").draggable("destroy")}catch(o){}}function floorTile_setTag(o){$("#floorTile_imagearea").empty();let e=$("#floorTile_password").val();for(item of ddf.images.imageList)ddf.images.tagInfos[item]?"（全て）"!=o&&!ddf.images.tagInfos[item].tags.includes(o)||""!=ddf.images.tagInfos[item].password&&ddf.images.tagInfos[item].password!=e||$("#floorTile_imagearea").append($(`<div><img src="${ddf.base_url+item}" /></div>`)):"（全て）"==o&&$("#floorTile_imagearea").append($(`<div><img src="${ddf.base_url+item}" /></div>`))}function floorTile_show(o){$("#map").addClass("floorTileEditing"),$(".floorTileFrame.dragprev").removeClass("dragprev"),$(".floorTileFrame.draggableObj").draggable(ddf.dragOption),$("#window_floorTile").show().css("zIndex",151),$(".draggable:not(#window_floorTile)").css("zIndex",150),$("#floorTile_image").show(),$("#floorTile_imageSelect").hide();var e;index=0,reg=/^(\d+)$/;for(item in ddf.characters)(v=reg.exec(ddf.characters[item].data.name))&&(index=Math.max(index,parseInt(v[1])));e={type:"floorTile",width:10,height:10,imgId:"",draggable:!0,imageUrl:"./image/defaultImageSet/floorTiles/floorTile_001.jpg",rotation:0,x:0,y:0},$("#floorTile_imgId").val(e.imgId),$("#floorTile_width").val(e.width),$("#floorTile_height").val(e.height),$("#floorTile_imageUrl").val(e.imageUrl),floorTile_previewUpdate()}function floorTile_previewUpdate(){zoom=Math.min(1,3/$("#floorTile_width").val(),3/$("#floorTile_height").val()),$("#floorTile_preview").css({width:50*$("#floorTile_width").val(),height:50*$("#floorTile_height").val(),transform:`scale(${zoom})`}),$("#floorTile_preview .inner").css({backgroundImage:"url("+ddf.base_url+$("#floorTile_imageUrl").val()+")"})}$("#btn_maptile").on("click",o=>{floorTile_show("")}),$("#window_floorTile input").on("change",floorTile_previewUpdate),$(document).on("click","#floorTile_imagearea div img",o=>{let e=$(o.currentTarget).attr("src");$("#floorTile_imageUrl").val(e.replace(ddf.base_url,"")),$("#floorTile_preview .inner").css("backgroundImage",`url(${e})`)}),$("#floorTile_imageChange").on("click",o=>{ddf.getImageTagsAndImageList().then(o=>{tagList=["フロアタイル画像"],ddf.images=o;for(item of ddf.images.imageList)if(ddf.images.tagInfos[item])for(tag of ddf.images.tagInfos[item].tags)""!=tag&&(tagList.includes(tag)||tagList.push(tag));tagList.push("（全て）"),$("#floorTile_tagbox").empty();for(item of tagList)$("#floorTile_tagbox").append($(`<option>${encode(item)}</option>`));$("#floorTile_tagbox").append($(`<option>${encode(item)}</option>`)),floorTile_setTag(tagList[0])}),$("#floorTile_image").hide(),$("#floorTile_imageSelect").show()}),ddf.cmd.floorTile_show=floorTile_show,$("#floorTile_btnpassword").on("click",o=>{$("#floorTile_btnpassword").hide(),$("#floorTile_password").show().focus()}),$("#floorTile_password").on("focusout",o=>{$("#floorTile_btnpassword").show(),$("#floorTile_password").hide(),floorTile_setTag($("#floorTile_tagbox").val())}).on("keydown",o=>{13==o.keyCode&&$("#floorTile_password").blur()}),$("#floorTile_tagbox").on("change",o=>{floorTile_setTag($("#floorTile_tagbox").val())}),$("#floorTile_close, #floorTile_close2").on("click",o=>{$("#window_floorTile").hide(),$(".floorTileFrame").addClass("dragprev"),safeDragDestroy(),ddf.cmd.safeDragDestroy(),$(".draggableObj").draggable(ddf.dragOption),$("#map").removeClass("floorTileEditing")});var click={x:0,y:0};$("#floorTile_preview").draggable({start:o=>{click.x=o.clientX,click.y=o.clientY},helper:()=>{let o=$("#floorTile_preview").clone();return o.addClass("floorTileFrame draggableObj"),o.css("transform",""),o.appendTo("#mapSurface"),o},drag:(o,e)=>{var i=ddf.roomState.zoom,l=e.originalPosition;e.position={left:(o.clientX-click.x+l.left)/i,top:(o.clientY-click.y+l.top)/i},ddf.roomState.viewStateInfo.isSnapMovablePiece&&(ddf.roomState.mapData.isAlternately&&ddf.roomState.mapData.gridInterval%2==1?1&Math.floor(e.position.top/50/ddf.roomState.mapData.gridInterval)?e.position={left:25*(1^(1|Math.floor(e.position.left/25))),top:50*Math.floor(e.position.top/50)}:e.position={left:25*(1|Math.floor(e.position.left/25)),top:50*Math.floor(e.position.top/50)}:e.position={left:50*Math.floor(e.position.left/50),top:50*Math.floor(e.position.top/50)})},stop:(o,e)=>{character={type:"floorTile",width:$("#floorTile_width").val(),height:$("#floorTile_height").val(),imageUrl:$("#floorTile_imageUrl").val(),imgId:"",draggable:!0,rotation:0,x:e.position.left/50,y:e.position.top/50},ddf.addCharacter(character)}});

},{}],41:[function(require,module,exports){
function getGraveyardCharacterData(){return ddf.getGraveyardCharacterData().then(a=>{$("#graveyard_characters").empty();for(item of a){switch(type=item.type,name=item.name,item.type){case"mapMask":type="マップマスク";break;case"Card":type="カード";break;case"CardZone":type="カード置き場";break;case"characterData":type="キャラクター";break;case"magicRangeMarker":type="魔法範囲";break;case"LogHorizonRange":type="ログホライズン攻撃範囲";break;case"MetallicGuardianDamageRange":type="メタリックガーディアン攻撃範囲";break;case"MagicTimer":type="魔法タイマー";break;case"mapMarker":name="",type="マップマーカー";break;case"floorTile":name="",type="フロアタイル";break;case"chit":name="",type="チット";break;case"Memo":type="共有メモ",name=item.message.split("\r")[0];break;case"diceSymbol":type="ダイスシンボル",name=`[${item.ownerName}]のダイス`;break;case"magicRangeMarkerDD4th":type="魔法範囲D&D4版"}$("#graveyard_characters").append($(`<option value="${item.imgId}">${encode(name)}[${type}]</option>`))}})}$("#btn_graveyard, #btn_graveyard2").on("click",a=>{getGraveyardCharacterData(),$("#window_graveyard").show().css("zIndex",151),$(".draggable:not(#window_graveyard)").css("zIndex",150)}),$("#graveyard_close, #graveyard_close2").on("click",a=>{$("#window_graveyard").hide()}),$("#graveyard_resurrect").on("click",a=>{ddf.resurrectCharacter($("#graveyard_characters").val()),$("#graveyard_characters")[0].remove($("#graveyard_characters")[0].selectedIndex)}),$("#graveyard_clear").on("click",a=>{ddf.clearGraveyard().then(a=>{getGraveyardCharacterData()})}),$("#graveyard_reload").on("click",a=>{getGraveyardCharacterData()});
...
},{"../.option.spectrum.json":17}],49:[function(require,module,exports){
...
},{"../.option.spectrum.json":17}],50:[function(require,module,exports){
function magicRangeMarker_show(a,e=0,r=0){if(character=ddf.characters[a])character=character.data,$("#window_magicRangeMarker .title").text("魔法範囲変更"),$("#magicRangeMarker_send").text("変更");else{index=0,reg=/^(\d+)$/;for(item in ddf.characters)(v=reg.exec(ddf.characters[item].data.name))&&(index=Math.max(index,parseInt(v[1])));character={type:"magicRangeMarker",name:index+1,rangeType:"circle",feets:5,color:0,timeRange:1,info:"",isHide:!1,size:0,x:e,y:r,counters:{},statusAlias:{},createRound:ddf.roomState.roundTimeData.round,draggable:!0,imageName:"",imgId:"0",initiative:ddf.roomState.roundTimeData.initiative,rotation:0,size:0},$("#window_magicRangeMarker .title").text("魔法範囲作成"),$("#magicRangeMarker_send").text("追加")}$("#magicRangeMarker_imgId").val(character.imgId),$("#magicRangeMarker_name").val(character.name),$("#magicRangeMarker_rangeType").val(character.rangeType),$("#magicRangeMarker_feets").val(character.feets),color=new tinycolor("rgb("+[character.color/65536&255,character.color/256&255,255&character.color]+")").toHex(),$("#magicRangeMarker_color").val(color),$("#magicRangeMarker_color2").spectrum("set","#"+color),$("#magicRangeMarker_timeRange").val(character.timeRange),$("#magicRangeMarker_info").val(character.info),$("#magicRangeMarker_isHide").prop("checked",!character.isHide),$("#window_magicRangeMarker").show().css("zIndex",151),$(".draggable:not(#window_magicRangeMarker)").css("zIndex",150)}$("#btn_rangedd3").on("click",a=>{ddf.cmd.magicRangeMarker_show("")}),$("#magicRangeMarker_close, #magicRangeMarker_close2").on("click",a=>{$("#window_magicRangeMarker").hide()}),sp_param=require("../.option.spectrum.json"),sp_param.change=(a=>{$("#magicRangeMarker_color").val(a.toHex())}),$("#magicRangeMarker_color2").spectrum(sp_param),ddf.cmd.magicRangeMarker_show=magicRangeMarker_show,$("#magicRangeMarker_send").on("click",a=>{(character=ddf.characters[$("#magicRangeMarker_imgId").val()])?(character.data.name=$("#magicRangeMarker_name").val(),character.data.rangeType=$("#magicRangeMarker_rangeType").val(),character.data.feets=$("#magicRangeMarker_feets").val(),character.data.color=parseInt("0x"+$("#magicRangeMarker_color").val()),character.data.timeRange=$("#magicRangeMarker_timeRange").val(),character.data.info=$("#magicRangeMarker_info").val(),character.data.isHide=!$("#magicRangeMarker_isHide").prop("checked"),ddf.changeCharacter(character.data).then(a=>{ddf.cmd.refresh_parseRecordData({record:[[0,"changeCharacter",[character.data],"dummy\t"]]}),$("#window_magicRangeMarker").hide()})):(character={type:"magicRangeMarker",size:0,x:1,y:1,counters:{},statusAlias:{},createRound:ddf.roomState.roundTimeData.round,draggable:!0,imageName:"",imgId:"0",initiative:ddf.roomState.roundTimeData.initiative,rotation:0,size:0},character.name=$("#magicRangeMarker_name").val(),character.rangeType=$("#magicRangeMarker_rangeType").val(),character.feets=$("#magicRangeMarker_feets").val(),character.color=parseInt("0x"+$("#magicRangeMarker_color").val()),character.timeRange=$("#magicRangeMarker_timeRange").val(),character.info=$("#magicRangeMarker_info").val(),character.isHide=!$("#magicRangeMarker_isHide").prop("checked"),ddf.addCharacter(character).then(a=>{$("#window_magicRangeMarker").hide(),ddf.cmd.initiative_sort(!0)}))});

},{"../.option.spectrum.json":17}],51:[function(require,module,exports){
function magicTimer_show(a){if(character=ddf.characters[a])character=character.data,$("#window_magicTimer .title").text("魔法タイマー変更"),$("#magicTimer_send").text("変更");else{index=0,reg=/^(\d+)$/;for(item in ddf.characters)(v=reg.exec(ddf.characters[item].data.name))&&(index=Math.max(index,parseInt(v[1])));character={type:"MagicTimer",name:index+1,createRound:ddf.roomState.roundTimeData.round,timeRange:1,initiative:ddf.roomState.roundTimeData.initiative,info:"",counters:{},statusAlias:{},imgId:"0",rotation:0},$("#window_magicTimer .title").text("魔法タイマー作成"),$("#magicTimer_send").text("追加")}$("#magicTimer_imgId").val(character.imgId),$("#magicTimer_name").val(character.name),$("#magicTimer_timeRange").val(character.timeRange),$("#magicTimer_createRound").val(character.createRound),$("#magicTimer_initiative").val(character.initiative),$("#magicTimer_info").val(character.info),$("#window_magicTimer").show().css("zIndex",151),$(".draggable:not(#window_magicTimer)").css("zIndex",150)}$("#btn_magictimer").on("click",a=>{ddf.cmd.magicTimer_show("")}),$("#magicTimer_close, #magicTimer_close2").on("click",a=>{$("#window_magicTimer").hide()}),ddf.cmd.magicTimer_show=magicTimer_show,$("#magicTimer_send").on("click",a=>{(character=ddf.characters[$("#magicTimer_imgId").val()])?(character.data.name=$("#magicTimer_name").val(),character.data.timeRange=$("#magicTimer_timeRange").val(),character.data.createRound=$("#magicTimer_createRound").val(),character.data.initiative=$("#magicTimer_initiative").val(),character.data.info=$("#magicTimer_info").val(),ddf.changeCharacter(character.data).then(a=>{ddf.cmd.refresh_parseRecordData({record:[[0,"changeCharacter",[character.data],"dummy\t"]]}),$("#window_magicTimer").hide()})):(character={type:"MagicTimer",counters:{},statusAlias:{},imgId:"0"},character.name=$("#magicTimer_name").val(),character.timeRange=$("#magicTimer_timeRange").val(),character.createRound=$("#magicTimer_createRound").val(),character.initiative=$("#magicTimer_initiative").val(),character.info=$("#magicTimer_info").val(),ddf.addCharacter(character).then(a=>{$("#window_magicTimer").hide(),ddf.cmd.initiative_sort(!0)}))});

},{}],52:[function(require,module,exports){
function mapChange_show(){switch($("#window_mapChange").show().css("zIndex",151),$(".draggable:not(#window_mapChange)").css("zIndex",150),color=new tinycolor("rgb("+[ddf.roomState.mapData.gridColor/65536&255,ddf.roomState.mapData.gridColor/256&255,255&ddf.roomState.mapData.gridColor]+")").toHex(),$("#mapChange_width").val(ddf.roomState.mapData.xMax),$("#mapChange_height").val(ddf.roomState.mapData.yMax),$("#mapChange_isAlternately").prop("checked",ddf.roomState.mapData.isAlternately),$("#mapChange_gridInterval").val(ddf.roomState.mapData.gridInterval),$("#mapChange_color").val(color),$("#mapChange_color2").spectrum("set","#"+color),ddf.roomState.mapData.mapType){case"imageGraphic":$("#mapChange_imageSource").val(ddf.roomState.mapData.imageSource),"image/whiteBack.png"==$("#mapChange_imageSource").val()?$("#mapChange_blank").prop("checked",!0):$("#mapChange_blank").prop("checked",!1),$("#mapChange_mirrored").prop("checked",ddf.roomState.mapData.mirrored)}mapChange_previewUpdate()}function mapChange_previewUpdate(){param={x:$("#mapChange_width").val(),y:$("#mapChange_height").val(),border:!0,alt:$("#mapChange_isAlternately").prop("checked"),num:!0,size:$("#mapChange_gridInterval").val(),color:"#"+$("#mapChange_color").val()},zoom=Math.min(1,7.26/param.y,8/param.x),$("#mapChange_preview").css("transform",`scale(${zoom})`),$("#mapChange_grid, #mapChange_map").css({width:50*param.x,height:50*param.y}),$("#mapChange_grid").attr("data","img/grid.svg?"+$.map(param,(a,e)=>e+"="+a).join("&")),$("#mapChange_map").attr("src",ddf.base_url+($("#mapChange_blank").prop("checked")?"image/whiteBack.png":$("#mapChange_imageSource").val())),$("#mapChange_mirrored").prop("checked")?$("#mapChange_map").addClass("mirrored"):$("#mapChange_map").removeClass("mirrored")}function mapChange_setTag(a){$("#mapChange_imagearea").empty();let e=$("#mapChange_password").val();for(item of ddf.images.imageList)ddf.images.tagInfos[item]?"（全て）"!=a&&!ddf.images.tagInfos[item].tags.includes(a)||""!=ddf.images.tagInfos[item].password&&ddf.images.tagInfos[item].password!=e||$("#mapChange_imagearea").append($(`<div><img src="${ddf.base_url+item}" /></div>`)):"（全て）"==a&&$("#mapChange_imagearea").append($(`<div><img src="${ddf.base_url+item}" /></div>`))}$("#btn_mapchange").on("click",a=>{mapChange_show()}),$("#window_mapChange input").on("change",mapChange_previewUpdate),sp_param=require("../.option.spectrum.json"),sp_param.change=(a=>{$("#mapChange_color").val(a.toHex()),mapChange_previewUpdate()}),$("#mapChange_color2").spectrum(sp_param),$("#mapChange_close, #mapChange_close2").on("click",a=>{$("#mapChange_image").show(),$("#mapChange_imageSelect").hide(),$("#window_mapChange").hide()}),$("#mapChange_imageChange").on("click",a=>{$("#mapChange_image").hide(),$("#mapChange_imageSelect").show(),ddf.getImageTagsAndImageList().then(a=>{tagList=["（全て）"],ddf.images=a;for(item of ddf.images.imageList)if(ddf.images.tagInfos[item])for(tag of ddf.images.tagInfos[item].tags)""!=tag&&(tagList.includes(tag)||tagList.push(tag));$("#mapChange_tagbox").empty();for(item of tagList)$("#mapChange_tagbox").append($(`<option>${encode(item)}</option>`));mapChange_setTag(tagList[0])})}),$("#mapChange_tagbox").on("change",a=>{mapChange_setTag($("#mapChange_tagbox").val())}),$(document).on("click","#mapChange_imagearea div img",a=>{let e=$(a.currentTarget).attr("src"),m=ddf.images.tagInfos[e.replace(ddf.base_url,"")].tags.join(" ");(match=/縦(\d+)横(\d+)/.exec(m))&&($("#mapChange_height").val(match[1]),$("#mapChange_width").val(match[2])),$("#mapChange_imageSource").val(e.replace(ddf.base_url,"")),$("#mapChange_blank").prop("checked",!1),$("#mapChange_mirrored").prop("checked",$("#mapChange_mirrored2").prop("checked")),mapChange_previewUpdate()}),$("#mapChange_btnpassword").on("click",a=>{$("#mapChange_btnpassword").hide(),$("#mapChange_password").show().focus()}),$("#mapChange_password").on("focusout",a=>{$("#mapChange_btnpassword").show(),$("#mapChange_password").hide(),mapChange_setTag($("#mapChange_tagbox").val())}).on("keydown",a=>{13==a.keyCode&&$("#mapChange_password").blur()}),$("#mapChange_send").on("click",a=>{ddf.changeMap("imageGraphic",$("#mapChange_blank").prop("checked")?"image/whiteBack.png":$("#mapChange_imageSource").val(),$("#mapChange_width").val(),$("#mapChange_height").val(),$("#mapChange_gridInterval").val(),$("#mapChange_isAlternately").prop("checked"),$("#mapChange_mirrored").prop("checked"),parseInt("0x"+$("#mapChange_color").val()),ddf.roomState.mapData.mapMarksAlpha,ddf.roomState.mapData.mapMarks).then(a=>{$("#mapChange_image").show(),$("#mapChange_imageSelect").hide(),$("#window_mapChange").hide()})});

},{"../.option.spectrum.json":17}],53:[function(require,module,exports){
function mapMarker_show(a){$("#window_mapMarker").show().css("zIndex",151),$(".draggable:not(#window_mapMarker)").css("zIndex",150);var r;if(null!=ddf.characters[a])r=ddf.characters[a].data,$("#mapMarker_change").show(),$("#mapMarker_create").hide(),$("#mapMarker_preview").addClass("edit"),$("#mapMarker_title").text("マップマーカー変更");else{index=0,reg=/^(\d+)$/;for(item in ddf.characters)(v=reg.exec(ddf.characters[item].data.name))&&(index=Math.max(index,parseInt(v[1])));r={type:"mapMarker",name:index+1,width:1,height:1,color:0,imgId:"",isPaint:!1,draggable:!0,rotation:0,message:"",x:0,y:0},$("#mapMarker_change").hide(),$("#mapMarker_create").show(),$("#mapMarker_preview").removeClass("edit"),$("#mapMarker_title").text("マップマーカー作成")}$("#mapMarker_imgId").val(r.imgId),color=new tinycolor("rgb("+[r.color/65536&255,r.color/256&255,255&r.color]+")").toHex(),$("#mapMarker_width").val(r.width),$("#mapMarker_height").val(r.height),$("#mapMarker_color").val(color),$("#mapMarker_color2").spectrum("set","#"+color),$("#mapMarker_message").val(r.message),$("#mapMarker_isPaint").prop("checked",r.isPaint),$("#mapMarker_draggable").prop("checked",!r.draggable),mapMarker_previewUpdate()}function mapMarker_previewUpdate(){zoom=Math.min(1,4.6/$("#mapMarker_width").val(),4.8/$("#mapMarker_height").val()),$("#mapMarker_preview").css("transform",`scale(${zoom})`),$("#mapMarker_preview").css({width:50*$("#mapMarker_width").val(),height:50*$("#mapMarker_height").val(),backgroundColor:$("#mapMarker_isPaint").prop("checked")?"#"+$("#mapMarker_color").val():""}),color=parseInt("0x"+$("#mapMarker_color").val()),refColor=[255-(color/65536&255),255-(color/256&255),255-(255&color)],$("#mapMarker_preview .message").text($("#mapMarker_message").val()),$("#mapMarker_preview .message").css({color:"rgb("+refColor+")"})}$("#btn_mapMarker").on("click",a=>{ddf.mapMarker_show("")}),$("#window_mapMarker .slider").slider({min:0,max:1,step:.05,stop:(a,r)=>{$("#mapMarker_alpha").val(r.value),mapMarker_previewUpdate()}}),$("#window_mapMarker input").on("change",mapMarker_previewUpdate),sp_param=require("../.option.spectrum.json"),sp_param.change=(a=>{$("#mapMarker_color").val(a.toHex()),mapMarker_previewUpdate()}),$("#mapMarker_color2").spectrum(sp_param),ddf.cmd.mapMarker_show=mapMarker_show,$("#mapMarker_close, #mapMarker_close2").on("click",a=>{$("#window_mapMarker").hide()});var click={x:0,y:0};$("#mapMarker_preview").draggable({start:a=>{click.x=a.clientX,click.y=a.clientY},helper:()=>{let a=$("#mapMarker_preview").clone();return a.appendTo("#mapSurface"),a},drag:(a,r)=>{var e=ddf.roomState.zoom,t=r.originalPosition;r.position={left:(a.clientX-click.x+t.left)/e,top:(a.clientY-click.y+t.top)/e},ddf.roomState.viewStateInfo.isSnapMovablePiece&&(ddf.roomState.mapData.isAlternately&&ddf.roomState.mapData.gridInterval%2==1?1&Math.floor(r.position.top/50/ddf.roomState.mapData.gridInterval)?r.position={left:25*(1^(1|Math.floor(r.position.left/25))),top:50*Math.floor(r.position.top/50)}:r.position={left:25*(1|Math.floor(r.position.left/25)),top:50*Math.floor(r.position.top/50)}:r.position={left:50*Math.floor(r.position.left/50),top:50*Math.floor(r.position.top/50)})},stop:(a,r)=>{character={type:"mapMarker",width:$("#mapMarker_width").val(),height:$("#mapMarker_height").val(),color:parseInt("0x"+$("#mapMarker_color").val()),message:$("#mapMarker_message").val(),isPaint:$("#mapMarker_isPaint").prop("checked"),imgId:"",draggable:!$("#mapMarker_draggable").prop("checked"),rotation:0,x:r.position.left/50,y:r.position.top/50},ddf.addCharacter(character),$("#mapMarker_multiple").prop("checked")||$("#window_mapMarker").hide()},cancel:".edit"}),$("#mapMarker_send").on("click",a=>{imgId=$("#mapMarker_imgId").val(),character=ddf.characters[imgId].data,character.color=parseInt("0x"+$("#mapMarker_color").val()),character.width=$("#mapMarker_width").val(),character.height=$("#mapMarker_height").val(),character.message=$("#mapMarker_message").val(),character.isPaint=$("#mapMarker_isPaint").prop("checked"),character.draggable=!$("#mapMarker_draggable").prop("checked"),ddf.changeCharacter(character).then(a=>{ddf.characters[imgId].data=character,ddf.cmd.refresh_parseRecordData({record:[[0,"changeCharacter",[character],"dummy\t"]]}),$("#window_mapMarker").hide()})});

},{"../.option.spectrum.json":17}],54:[function(require,module,exports){
...
},{"../.option.spectrum.json":17}],55:[function(require,module,exports){
...
function uploadFile_uploadfiles(e){for(file of e)new Promise((e,a)=>{let l=new FileReader;l.onload=e,l.readAsArrayBuffer(file)}).then((e=>a=>{ddf.uploadFile(e.name,ddf.base_url,new Uint8Array(a.target.result)).then(e=>{"OK"==e.resultText?ddf.sendChatMessage(0,"どどんとふ\t",`${ddf.userState.name}がファイルをアップロードしました \n  ファイル名：${e.uploadFileInfo.fileName}\n  URL:${e.uploadFileInfo.fileUploadUrl}`,"00aa00",!0):console.log(e.resultText)})})(file));$("#window_uploadFile").hide()}$("#btn_easyUpload").on("click",e=>{$("#window_uploadFile").show().css("zIndex","151"),$(".draggable:not(#window_uploadFile)").css("zIndex","150")}),$("#uploadFile_close, #uploadFile_close2").on("click",e=>{$("#window_uploadFile").hide()}),$("#window_uploadFile :file").on("change",e=>{arr=[];for(item of $("#window_uploadFile :file")[0].files)arr.push(item);uploadFile_uploadfiles(arr)});let $form=$("#uploadFile_droparea");(e=>{$("#uploadFile_droparea ~ .overwrap").on("dragenter",()=>{e.addClass("is-dragover")}),e.on("drag dragstart dragend dragover dragenter dragleave drop",e=>{e.preventDefault(),e.stopPropagation()}).on("dragover dragenter",()=>{e.addClass("is-dragover")}).on("dragleave dragend drop",()=>{e.removeClass("is-dragover")}).on("drop",e=>{droppedFiles=e.originalEvent.dataTransfer.files,uploadFile_uploadfiles(droppedFiles)})})($form),$("#window_uploadFile .overwrap a").on("click",e=>($("#window_uploadFile .overwrap :file").click(),!1));
...
},{"../../../package.json":16}],62:[function(require,module,exports){
function vote_alerm_show(){$("#window_vote_alerm").show().css("zIndex",151),$(".draggable:not(#window_vote_alerm)").css("zIndex",150)}function vote_select_show(e){$("#vote_select_text").text(e),$("#window_vote_select").show().css("zIndex",151),$(".draggable:not(#window_vote_select)").css("zIndex",150)}$("#btn_vote").on("click",e=>{$("#vote_requiredCount").val(1),$("#vote_alerm").prop("checked",!0),$("#vote_isSelect").hide(),$("#vote_number").text(ddf.roomState.loginUserInfo.length-1),$("#vote_question").val(""),$("#window_vote").show().css("zIndex",151),$(".draggable:not(#window_vote)").css("zIndex",150)}),$("#window_vote [name=vote]").on("click",e=>{$("#vote_alerm").prop("checked")?$("#vote_isSelect").hide():$("#vote_isSelect").show()}),$("#vote_send").on("click",e=>{$("#vote_alerm").prop("checked")?ddf.sendChatMessage(0,ddf.userState.name+"\t",`###vote###{"question":"","isCallTheRoll":true,"callerId":"${ddf.info.uniqueId}","requiredCount":${$("#vote_requiredCount").val()}}`,ddf.userState.chatColor):ddf.sendChatMessage(0,ddf.userState.name+"\t",`###vote###{"question":"${$("#vote_question").val()}","isCallTheRoll":false,"callerId":"${ddf.info.uniqueId}","requiredCount":${$("#vote_requiredCount").val()}}`,ddf.userState.chatColor),$("#window_vote").hide()}),$("#vote_close, #vote_close2").on("click",e=>{$("#window_vote").hide()}),ddf.cmd.vote_alerm_show=vote_alerm_show,ddf.cmd.vote_select_show=vote_select_show,$("#vote_alerm_ok").on("click",e=>{ddf.sendChatMessage(0,ddf.userState.name+"\t",'###vote_replay_readyOK###{"voteReplay":4,"isCallTheRoll":true}',ddf.userState.chatColor),$("#window_vote_alerm").hide()}),$("#vote_select_agree").on("click",e=>{ddf.sendChatMessage(0,ddf.userState.name+"\t",'###vote_replay_readyOK###{"voteReplay":1,"isCallTheRoll":false}',ddf.userState.chatColor),$("#window_vote_select").hide()}),$("#vote_select_disagree").on("click",e=>{ddf.sendChatMessage(0,ddf.userState.name+"\t",'###vote_replay_readyOK###{"voteReplay":2,"isCallTheRoll":false}',ddf.userState.chatColor),$("#window_vote_select").hide()});
